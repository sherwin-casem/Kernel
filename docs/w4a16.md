...
backend). The result: it **beats cuBLAS FP16 across the decode regime** (~1.18-1.33x at
batch size 1, 1.10-1.21x at batch size 8, on FFN and attention projections) via an
autotuned split-K dispatch, and matches FP16 for larger batches, with no vendor-specific
code.
...
Autotuned split-K dispatch (`M <= 8`); speedup at M=1 / M=8.

| Shape (K, N) | W4A16 M=1 | FP16 M=1 | M=1 | M=8 |
|---|---|---|---|---|
| qwen2-72b-ffn (8192, 29568) | 0.242 ms | 0.311 ms | **1.28x** | **1.21x** |
| llama3-8b-attn-qkv (4096, 4096) | 0.035 ms | 0.047 ms | **1.33x** | **1.14x** |
| llama3-8b-ffn-up (4096, 14336) | 0.085 ms | 0.100 ms | **1.18x** | **1.10x** |

W4A16 beats FP16 across the decode regime (small M) on both FFN and attention
projections, and cedes to FP16 as M grows into the compute-bound (prefill) regime (e.g.
qwen M=128: 0.46x). It also carries a 4x weight-memory reduction at all M. The split-K
config (`SPLIT_K`, `BLOCK_N`, stages, warps) is autotuned per shape, because the optimum
is non-monotonic in the shape (attn-qkv wants `SPLIT_K=8`, llama-ffn 4, qwen 8).
...
beat FP16 (1.18-1.33x) there. As M grows the operating point marches up the memory roofline and past the
